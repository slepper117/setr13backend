  ...
  try {
    ...
    const newClock = await query('clocks').insert(
      ...
      [
        'id',
        query.raw(
          '(SELECT row_to_json(x) FROM (SELECT u.id, u.name FROM setr.users u WHERE clocks.id_user = u.id) x) AS user'
        ),
        'direction',
        'datetime',
        query.raw(
          '(SELECT row_to_json(x) FROM (SELECT u.id, u.name FROM setr.users u WHERE clocks.log = u.id) x) AS log'
        ),
      ]
    );
...
const list = async (req, res, next) => {
  ...
  const { user, direction, orderby, order, year, month, day } = req.query;
  let listClocks = [];
