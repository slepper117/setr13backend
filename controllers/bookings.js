  ...
  try {
    ...
    const newBooking = await query('bookings').insert(
      ...
      [
        'id',
        query.raw(
          '(SELECT row_to_json(x) FROM (SELECT u.id, u.name FROM setr.users u WHERE bookings.id_user = u.id) x) AS user'
        ),
        query.raw(
          '(SELECT row_to_json(x) FROM (SELECT r.id, r.name FROM setr.rooms r WHERE bookings.id_room = r.id) x) AS room'
        ),
        'start',
        'final',
