  ...
  try {
    ...
    const getNewUser = await query
      .select(
        'id',
        query.raw(
          '(SELECT row_to_json(x) FROM (SELECT ur.id, ur.name FROM setr.user_roles ur WHERE users.id_user_role = ur.id) x) AS role'
        ),
        'username',
        'name',
        'tag',
        'schedule',
        query.raw(
          "coalesce((SELECT array_to_json(array_agg(row_to_json(x))) FROM (SELECT a.id, a.name FROM setr.areas a JOIN setr.users_areas ua ON a.id = ua.id_area WHERE users.id = ua.id_user) x), '[]') AS areas"
        ),
        query.raw(
          "coalesce((SELECT array_to_json(array_agg(row_to_json(x))) FROM (SELECT c.id, c.direction, c.datetime FROM setr.clocks c WHERE users.id = c.id_user LIMIT 4) x), '[]') AS lastClocks"
        ),
        query.raw(
          "coalesce((SELECT array_to_json(array_agg(row_to_json(x))) FROM (SELECT b.id, b.start, b.description FROM setr.bookings b WHERE users.id = b.id_user AND b.start >= NOW()::timestamp ORDER BY b.start ASC) x), '[]') AS nextBookings"
        )
      )
      .from('users')
